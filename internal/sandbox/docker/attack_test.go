...
func TestAttackOutboundNetworkFailsInAgentPhase(t *testing.T) {
	h := newHarness(t)
	// A TCP connect, a DNS lookup, the routing table (header line only when
	// there is no route out) and the interface list, so a resolver-only or
	// interface-only leak would also show. The kernel's tunnel stubs (gre0,
	// sit0, ...) appear in every namespace and are not interfaces out.
	...
	t.Run("network none from the start", func(t *testing.T) {
	...
	t.Run("registries during install, none after disconnect", func(t *testing.T) {
		sb := h.create(t, h.spec(t, contracts.NetworkRegistries))
		if nets := h.inspect(t, sb).NetworkSettings.Networks; len(nets) != 1 {
			t.Fatalf("install phase should be on exactly one network, got %v", nets)
		}
		if ifaces := h.out(t, sb, "ls /sys/class/net"); !strings.Contains(ifaces, "eth0") {
			t.Fatalf("install phase has no interface: %q", ifaces)
		}

		if err := sb.DisconnectNetwork(h.ctx); err != nil {
			t.Fatalf("DisconnectNetwork: %v", err)
		}
		if err := sb.DisconnectNetwork(h.ctx); err != nil {
			t.Fatalf("second DisconnectNetwork: %v", err)
		}
		if got := h.out(t, sb, probe); got != cut {
			t.Errorf("network reachable after disconnect:\n%s", got)
		}
		assertNoNetworks(t, h.inspect(t, sb).NetworkSettings.Networks)
...
func TestAttackFileOperationsRefuseSymlinks(t *testing.T) {
	...
	for _, path := range []string{"leak", "escape/target", "relative/target"} {
		if data, err := sb.ReadFile(h.ctx, path); err == nil {
			t.Errorf("ReadFile(%q) followed a symlink and returned %q", path, data)
		}
		if err := sb.WriteFile(h.ctx, path, []byte("x")); err == nil {
			t.Errorf("WriteFile(%q) wrote through a symlink", path)
		}
	}
